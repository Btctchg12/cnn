...
def adapt_first_conv_for_multichannel(conv_layer, in_channels):
    """
    Adapt pretrained ResNet first conv layer from 3 input channels
    ...
    with torch.no_grad():
        if in_channels == 3:
            ...
            # Average pretrained RGB weights and repeat them for all input channels.
...
class ResNet18Classifier(nn.Module):
    def __init__(self, in_channels, num_classes=3, pretrained=True):
        ...
        # Change first conv layer from 3 channels to tif input channels.
